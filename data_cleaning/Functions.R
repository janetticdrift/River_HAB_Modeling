...
##############################
#####Toxin Simulations for Within-Mat TM Mats
##############################
#This function simulates then summarizes one year of data at a time
#The model output includes Anabaena(t-2) in both the hurdle and the entire model

simulate_toxin_mat <- function(
    x,
    microscopy_latent,
    year,
    start_timestep,
  ...
  microscopy_use <- microscopy_latent[year_starttime,,drop = FALSE]
  ...
  X1 <- as.matrix(cbind(intercept = rep(1, time),
                        microscopy_use[,-1],  # Abundances are log-transformed
                        nitrate = nitrate_use,
                        phos = phos_use,
                        amon = amon_use,
  ...
  for (z in 1:runs) {
    ...
    for (t in 3:time) {
      ...
      phi_t <- plogis(Phi0[z] + PhiAna[z]*X1[t-2, 2])
  ...
  matsims <- dplyr::left_join(simsmedian, simslquant, by = "time") %>% 
  ...
  return(list(predictions = matsims,
              raw = tox))
...
##############################
#####Toxin Simulations for No-Lag River-Wide TM Mats
##############################
#This function simulates then summarizes one year of data at a time
#The model does not include an Anabaena t-2 lag

simulate_toxin_nolag <- function(
    x,
    percentcover_latent,
    year,
    start_timestep,
  ...
  for (z in 1:runs) {
    ...
    beta <- beta_matrix[z, ]
      ...
      for (t in 3:time) {
        ...
        #----------
        #Process: How much toxin is created?
        #----------
