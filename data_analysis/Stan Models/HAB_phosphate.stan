...
data {
  ...
  vector [uniqueID] phos; //Vector of phosphate levels
...
parameters {
  ...
  vector[Nspecies] Ptheta; //parameter for phosphate each week 
  ...
  for(t in 2:uniqueID){
    for(s in 1:Nspecies){
      ...
      if(firstdays[t]==1) continue;
       n[s,t] ~ normal(Alpha[s] + Beta[s]*n[s, t-1] + Ptheta[s]*phos[t-1] +
